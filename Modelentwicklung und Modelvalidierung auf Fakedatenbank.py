...
medications_cost = pd.read_sql_query("SELECT PATIENT, SUM(TOTALCOST) as TOTAL_MED_COST FROM medications GROUP BY PATIENT;", conn)
procedures_count = pd.read_sql_query("SELECT PATIENT, COUNT(*) as PROCEDURE_COUNT FROM procedures GROUP BY PATIENT;", conn)
procedures_cost = pd.read_sql_query("SELECT PATIENT, SUM(BASE_COST) as TOTAL_PROC_COST FROM procedures GROUP BY PATIENT;", conn)
encounter_cost = pd.read_sql_query("SELECT PATIENT, SUM(TOTAL_CLAIM_COST) as TOTAL_ENC_COST FROM encounters GROUP BY PATIENT;", conn)
payer_coverage = pd.read_sql_query("SELECT PATIENT, AVG(PAYER_COVERAGE) as AVG_PAYER_COV FROM encounters GROUP BY PATIENT;", conn)
...
final_df = merged_df[["HEALTHCARE_EXPENSES", "BMI", "HEALTHCARE_COVERAGE", "AGE", "TOTAL_MED_COST", "PROCEDURE_COUNT", "TOTAL_PROC_COST", "TOTAL_ENC_COST", "AVG_PAYER_COV"]]
...
chronic_conditions_list = ["None", "Diabetes", "COPD", "Cancer", "Hypertension", "Depression", "Multiple Conditions"]
...
for i in range(1, num_patients + 1):
    # Alter simulieren
    birthdate = fake.date_of_birth(minimum_age=20, maximum_age=90).strftime("%Y-%m-%d")
    age = 2025 - int(birthdate[:4])

    # Versicherungsschutz (jüngere Menschen haben oft schlechtere Abdeckung)
    healthcare_coverage = np.clip(np.random.normal(0.7 - (age / 200), 0.2), 0.1, 1.0)

    # Einkommen simulieren (höheres Alter → leicht sinkendes Einkommen)
    income = np.clip(np.random.normal(50000 - (age * 300), 15000), 15000, 150000)
    ...
    employment_status = np.random.choice(employment_status_list, p=[0.1, 0.6, 0.2, 0.1] if age < 65 else [0.05, 0.2, 0.15, 0.6])
    ...
    chronic_condition = np.random.choice(
        ...
        p=[0.5 - (age / 200), 0.2, 0.1, 0.05, 0.1, 0.05, 1.0 - (0.5 - (age / 200) + 0.2 + 0.1 + 0.05 + 0.1 + 0.05)]
    )

    # Krankenhausaufenthalte steigen mit Alter & Vorerkrankungen
    hospitalizations = np.clip(np.random.poisson(0.5 + (0.3 if chronic_condition != "None" else 0) + (age / 80)), 0, 10)

    # Raucherstatus (ältere Menschen rauchen häufiger in historischen Daten)
    smoker = np.random.choice([0, 1], p=[0.75, 0.25] if age > 50 else [0.85, 0.15])

    # Notaufnahme-Besuche steigen mit Alter & Rauchen
    emergency_visits = np.clip(np.random.poisson(0.2 + (smoker * 0.2) + (hospitalizations * 0.1)), 0, 5)

    # Medikamentenkosten hängen von Alter, BMI & Vorerkrankungen ab
    medication_cost = np.clip(np.random.normal(500 + (age * 10) + (bmi * 5) + (hospitalizations * 200), 500), 0, 15000)

    # Behandlungskosten steigen mit Alter, Vorerkrankungen & Raucherstatus
    encounter_cost = np.clip(np.random.normal(2000 + (age * 20) + (bmi * 10) + (hospitalizations * 500) + (smoker * 1000), 1000), 1000, 70000)

    # Gesundheitskosten hängen von Versicherung, Einkommen, Vorerkrankungen & Raucherstatus ab
    base_expenses = encounter_cost + medication_cost + (hospitalizations * 5000) + (emergency_visits * 2000)
    healthcare_expenses = np.clip(base_expenses * (1 - healthcare_coverage) * (1 + smoker * 0.2) * (1 - income / 200000), 5000, 200000)

    # Daten speichern
    patients_data.append((healthcare_expenses, healthcare_coverage, birthdate, income, chronic_condition, hospitalizations, smoker, gender, employment_status, emergency_visits))
...
cursor.executemany("INSERT INTO observations (PATIENT, VALUE, CODE) VALUES (?, ?, ?)", observations_data)
cursor.executemany("INSERT INTO medications (PATIENT, TOTALCOST) VALUES (?, ?)", medications_data)
cursor.executemany("INSERT INTO encounters (PATIENT, TOTAL_CLAIM_COST) VALUES (?, ?)", encounters_data)
...
# Daten abrufen
patients_df = pd.read_sql_query("SELECT Id, HEALTHCARE_EXPENSES, HEALTHCARE_COVERAGE, BIRTHDATE, INCOME, CHRONIC_CONDITIONS, HOSPITALIZATIONS, SMOKER, GENDER, EMPLOYMENT_STATUS, EMERGENCY_VISITS FROM patients;", conn)
bmi_values = pd.read_sql_query("SELECT PATIENT, VALUE as BMI FROM observations WHERE CODE = '39156-5';", conn)
medications_cost = pd.read_sql_query("SELECT PATIENT, SUM(TOTALCOST) as TOTAL_MED_COST FROM medications GROUP BY PATIENT;", conn)
encounter_cost = pd.read_sql_query("SELECT PATIENT, SUM(TOTAL_CLAIM_COST) as TOTAL_ENC_COST FROM encounters GROUP BY PATIENT;", conn)
...
merged_df["EMPLOYMENT_STATUS"] = merged_df["EMPLOYMENT_STATUS"].astype("category").cat.codes
merged_df["CHRONIC_CONDITIONS"] = merged_df["CHRONIC_CONDITIONS"].astype("category").cat.codes
